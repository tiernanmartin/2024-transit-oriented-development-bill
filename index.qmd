...
ps_cities <- pop_hu_change_cities |> 
...
cs_awmndc <- split(case_study_results, city_awmndc$city) |> 
  map(\(x) pluck(x,"awm_ndc") |> round(digits = 2))
...
We find the area-weighted mean net development capacity ($AWM_{NDC}$) for Kent's station areas to be `r cs_awmndc['Kent']` FAR. This metric summarizes HB 2160's effect on the *Developable, Affected* parcels in the city's station areas, which includes 69% of the station area parcels and 62% of the station area land.
