...
In order to get a sense for how the effects of HB 2160 may differ from place to place, we examine several cities as case studies. 
...
st_join(t,ps_cities) |> 
  ...
  select(-geoid) |> 
  select(name, population, housing_units, everything()) |> 
  gt(rowname_col = "name") |> 
   tab_spanner(
    label = "Population & Housing",
    columns = c(population, housing_units)
  ) |>
  tab_spanner(
...
*Insert table with population (2023), housing units (2023), land area, and count of each type of transit station*
