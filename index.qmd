...
#### Total Net Development Capacity ($T_{NDC}$) {.unnumbered}
...
#### Area-Weighted Mean of Net Development Capacity ($AWM_{NDC}$) {.unnumbered}
...
In a region where cities employ various tools to regulate development intensity, our method introduces two metrics for assessing changes in development capacity, irrespective of the regulatory tool in use. It also enables us to summarize the bill's effect at different geographic levels, including station areas, cities, and regions. This approach allows us to describe individual geographies (for example, a specific station area) and to compare them with one another (for example, several cities in comparison to the region as a whole).
...
dev_cap_total <- analysis_study_group |> 
  filter(str_detect(name, "total")) |> 
  pluck("devcap_tbl") |> 
  str_replace("B", " billion")
...
1.  *Not Developable* (${i | I_i = 0}$)\
    Parcels ineligible under HB 2160 criteria. This study applies the HB 2160 criteria by selecting parcels that: a) are located within cities and within a UGA; b) are located in zoning districts permitting residential uses; and c) have current land uses that are suitable for transit-oriented development. To evaluate current land use, we use tax assessor-assigned land use codes to exclude properties with incompatible uses. HB 2160 specifies incompatible uses as streets, roads, highways, sidewalks, lighting systems for streets and roads, traffic signals, domestic water systems, storm and sanitary sewer systems, parks and recreational facilities, and schools [@reed2024]. Although HB 2160 exempts parcels governed by shoreline master programs or critical area ordinances, this study does not exclude these lots due to the lack of comprehensive regional data sets with the required information.

2.  *Developable* (${i | I_i = 1}$)\
    Parcels that meet 2160's eligibility criteria. Within *Developable* parcels, we distinguish the following two subsets:

    A.  *Developable, Not Affected* ($\{i | I_i = 1 \text{ and } NDC_i \leq 0\}$)\
        Parcels that meet the eligibility criteria and have a standardized development capacity equal or greater than the minimum required by HB 2160 and would, therefore, not be affected by the bill.
    B.  *Developable, Affected* ($\{i | I_i = 1 \text{ and } NDC_i \gt 0\}$)\
        Parcels that meet the eligibility criteria and have a standardized development capacity less than the minimum required by HB 2160 and would, therefore, be affected by the bill.
...
Using HB 2160's definition of station areas, we create two station area types and assign one type to every parcel. Parcels that are within both *Large* and *Small* station area types as classified as part of a *Large* station area.[^8]

[^8]: While many parcels are within more than one type of station area, our analysis sets a limit of one station area type per parcel to ensure that each parcel is only counted once during tabulation and aggregation procedures.

Our study focuses primarily on the third analysis group, *Developable, Affected*, which make up the majority of the station area parcels (`r dev_aff_n_pct`) but constitute only `r dev_aff_area_pct` of the total land area. We find that the total amount of additional development capacity that would be added by HB 2160 to be approximately `r dev_cap_total` ft^2^.[^9]

[^9]: While increasing development capacity is necessary for the production of additional housing units, it is not a sufficient condition.
...
analysis_study_group |> 
  ...
  grand_summary_rows(
    fns =  list(label = "All Station Area Parcels", id = "totals") ~ gt_custom_summary(., df = analysis_study_group), 
    side = "top"
  ) |> 
  tab_style(
    style = list(
...
We use the area-weighted mean net development capacity ($AWM_{NDC}$) to summarize HB 2160's effect across geographic scales. The $AWM_{NDC}$ for the central Puget Sound region is `r total_awmndc` FAR. We intrepret this result to mean that, after adjusting for land area, the typical station area parcel is likely to have slightly more development capacity than what HB 2160 would introduce. By plotting the distribution of net development capacity, we can observe that while a significant amount of land would be affected (the histogram bars to the right of the red dashed line), there is a "long tail" of land that is already zoned for high development capacity in these station areas (the bars to the left of the dashed line). See @fig-results-histogram below.

```{r}
#| label: fig-results-histogram
#| output: true
#| message: false
#| fig-cap: "Distribution of Net Development Capacity"
#| fig-height: 4

tar_load(analysis_parcels_ndc)

awmndc_dev_total <- analysis_study_group |> 
  filter(value == "Total") |> 
  pluck("awmndc_tbl")
...
s <- glue("Includes all 'Developable' parcels, $AWM_{{NDC}}$ = {awmndc_dev_total} FAR, land area = {area_dev_aff_mi} $mi^2$.")
...
analysis_parcels_ndc |> 
  ...
  scale_x_continuous(limits = c(-10,4),
                     breaks = c(-10,-5,0,1,2,3,4)) + 
  labs(title = "Distribution of Net Development Capacity",
       subtitle = subtitle,
...
However, for parcels where the current development capacity is less than what HB 2160 would require, we observe that the majority would be significantly impacted by the bill. @fig-results-histogram-affected plots the distribution of net development capacity ($NDC$) for all *Developable, Affected* parcels, revealing that most of the land in this group of station area parcels would be affected by a development capacity increase of 1.5 FAR or greater.

```{r}
#| label: fig-results-histogram-affected
#| output: true
#| message: false
#| fig-cap: "Distribution of Net Development Capacity in Affected Parcels"
#| fig-height: 4
...
analysis_parcels_ndc |> 
  ...
  scale_x_continuous(breaks = seq(0, ndc_uni_max, by = 0.5),
                     limits = c(0,ndc_uni_max)) + 
  labs(title = "Distribution of Net Development Capacity in Affected Parcels",
       subtitle = subtitle,
       y = expression("Land Area (mi"^2*")"),
       x = "Net Development Capacity (FAR)") 
```
...
```{r}
#| label: results-station-area-types-awmndc

tar_load(analysis_station_types_dev_affected)

large_awmndc <- analysis_station_types_dev_affected |> 
  filter(str_detect(analysis_station_area_types, "Large")) |>
  pluck("awmndc_tbl")

small_awmndc <- analysis_station_types_dev_affected |> 
  filter(str_detect(analysis_station_area_types, "Small")) |>
  pluck("awmndc_tbl")

```

In @fig-results-histogram-station-areas, we compare the distribution of $NDC$ for *Developable, Affected* parcels between the two station area types. The $AWM_{NDC}$ for the *Large* station areas is {`r large_awmndc`} FAR and $AWM_{NDC}$ for the *Small* station areas is {`r small_awmndc`} FAR.
...
s <- glue("Includes all 'Developable, Affected' parcels, Large Station Area $AWM_{{NDC}}$ is {large_awmndc} FAR, Small Station Area $AWM_{{NDC}}$ is {small_awmndc}")
...
tar_load(analysis_stations_awmndc)
tar_load(analysis_transit_walksheds)

t_stop_name <- analysis_transit_walksheds |> 
    mutate(stop_name = as.character(glue("{stop_name} (ID:{row_number()})")))

t_sf <- t_stop_name |> 
  select(stop_name) |> 
  left_join(analysis_stations_awmndc)
  
my_pal <- viridisLite::viridis(n = 6) |> 
  rev() |> 
  set_names(levels(t_sf$without_averaging))

mapview(t_sf, 
        zcol = "without_averaging", 
        layer.name = "Net Dev. Capacity",
        col.region = my_pal)
...
tar_load(pop_hu_change_cities)
tar_load(pop_hu_change_cities_no_water)
...
case_study_cities <- pop_hu_change_cities |> 
...
case_study_cities_no_water <- pop_hu_change_cities_no_water |> 
  filter(name %in% case_studies) |> 
  st_transform(2926) |> 
  mutate(area = st_area(geometry),
         area_mi2 = as.numeric(convert_to_mi2(area))
...
st_join(case_study_cities) |> 
  ...
  mutate(SC = 0L) |> 
  left_join(st_drop_geometry(case_study_cities)) |> 
  select(-geoid, -area) |> 
  select(name, population, housing_units, area_mi2, everything()) |> 
  gt(rowname_col = "name") |> 
   tab_spanner(
    label = "Population, Housing, & Land",
...
cs_results_analysis_group <- 
analysis_parcels_ndc |> 
  st_drop_geometry() |> 
  filter(city %in% case_studies) |> 
  group_by(city, analysis_type_uni) |> 
  summarize(.groups = "drop",
            n = n(),
            area = 
              round(digits = 2,
                    sum(area_mi2, na.rm = TRUE))
            ) |> 
  group_by(city) |> 
  ...
  summarize(
    ...
    n_pct = percent(n/n_total),
    area_pct = percent(area/area_total),
    n_tbl = glue("{comma(n)} ({percent(n/n_total)})"),
    area_tbl = glue("{area} ({percent(area/area_total)})")
  ) 

cs_results_analysis_group |>
  select(city, analysis_type_uni, ends_with("tbl")) |> 
  gt(rowname_col = "analysis_type_uni" ,
     groupname_col = "city",
     process_md = TRUE) |> 
  cols_label(
    n_tbl = "Parcels (n)",
...
case_study_results <- analysis_parcels_ndc |> 
  st_drop_geometry() |> 
  filter(city %in% case_studies)|>
  filter(analysis_type_avg %in% "Developable") |> 
  group_by(city) |> 
  summarise(.groups = "drop",
            n = n(),
            area = sum(area_mi2, na.rm = TRUE),
            awm_ndc = weighted_mean(x = analysis_ndc,
                                    w = area_mi2),
            area_ndc_ft2 = sum(analysis_dev_cap_added, na.rm = TRUE)) 
...
case_study_results |> 
  ...
  cols_label(
    ...
    awm_ndc = html("<i>AWM<sub>NDC</sub></i>"),
  ...
  tab_footnote(
    footnote = "Area-weighted mean of net development capacity",
    locations = cells_column_labels(
      columns = 4
    )
  ) |> 
  tab_source_note(source_note = "Note: This table summarizes all Developable parcels within each city's station areas; it does not include parcels that would be exempt from HB 2160.")
...
cs_results_analysis_group_dev <-
analysis_parcels_ndc |> 
  st_drop_geometry() |> 
  filter(city %in% case_studies) |> 
  group_by(city, analysis_type_avg) |> 
  summarize(.groups = "drop",
            n = n(),
            area = 
              round(digits = 2,
...
cs_results_analysis_group_devaffected <-
analysis_parcels_ndc |> 
  st_drop_geometry() |> 
  filter(city %in% case_studies) |> 
  filter(analysis_type_uni %in% c("Developable, Affected")) |> 
  group_by(city, analysis_type_uni) |> 
  summarize(.groups = "drop",
            n = n(),
            area = 
...
cs_results_analysis_group_dev_only <- cs_results_analysis_group_dev |> 
  filter(analysis_type_avg %in% "Developable")

cs_n <- cs_results_analysis_group_dev_only |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"n") |> comma())

cs_area <- cs_results_analysis_group_dev_only |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"area"))

cs_n_pct <- cs_results_analysis_group_dev_only |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"n_pct"))

cs_area_pct <- cs_results_analysis_group_dev_only |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"area_pct"))


cs_devaff_n <- cs_results_analysis_group_devaffected |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"n") |> comma())

cs_devaff_area <- cs_results_analysis_group_devaffected |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"area"))

cs_devaff_n_pct <- cs_results_analysis_group_devaffected |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"n_pct"))

cs_devaff_area_pct <- cs_results_analysis_group_devaffected |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"area_pct"))


cs_awmndc <- case_study_results |> 
  split(case_study_results$city) |> 
  map(\(x) pluck(x,"awm_ndc") |> 
        round(digits = 2))
...
analysis_parcels_ndc |> 
  ...
  summarise(.groups = "drop",
            ...
            awm_ndc = weighted_mean(x = analysis_ndc,
                                    w = area_mi2),
            area_ndc_ft2 = sum(analysis_dev_cap_added, na.rm = TRUE)) |> 
  mutate(awm_ndc = if_else(is.nan(awm_ndc),0,awm_ndc))

max_ndc <- max(na.rm = TRUE, analysis_parcels_ndc$analysis_ndc_uniform)
  

```

```{r}
#| label: results-case-studies-transit-stations

tar_load(analysis_transit_walksheds)

case_studies_transit_stations <- analysis_transit_walksheds |> 
  st_join(case_study_cities) |> 
  transmute(
    name,
    mode = factor(mode,
...
t_kent <- case_studies_transit_stations |> 
  filter(name %in% "Kent")
...
kent <- 
  ...
  filter(name %in% "Kent") |> 
  st_cast("POLYGON") |> 
  mutate(area_polygon = st_area(geometry)) |> 
  slice_max(area_polygon,n = 1) |> # keep only the main part of Kent
  select(-area_polygon)

p_kent <- analysis_parcels_ndc |> 
  filter(city %in% "Kent") |>   
  filter(analysis_type_avg %in% "Developable") |> 
  select(area_mi2,
         district_name,
         net_dev_capacity = analysis_ndc,
         analysis_type_avg,
         analysis_type_uni)
...
We find the area-weighted mean net development capacity ($AWM_{NDC}$) for Kent's station areas to be `r cs_awmndc['Kent']` FAR. This metric summarizes HB 2160's effect on the *Developable* parcels in the city's station areas, which includes `r cs_n_pct['Kent']` of the station area parcels (`r cs_n_pct['Kent']` parcels) and `r cs_area_pct['Kent']` of the station area land (`r cs_area['Kent']` mi^2^).

In @fig-results-case-studies-kent-histogram, we observe the distribution of changes in development capacity that HB 2160 would have on *Developable, Affected* parcels in Kent's station areas.
...
kent_hist_subtitle <- "Area-weighted mean net development capacity ($AWM_{{NDC}}$) is {cs_awmndc['Kent']} FAR" |> 
  glue() |> 
  TeX()
...
The majority of parcels within this city's station areas is zoned as "Single-Family". The total net development capacity for this zoning district would be a significant increase, as shown in @tbl-results-case-studies-kent-by-district.


```{r}
#| label: tbl-results-case-studies-kent-by-district
#| message: false
#| tbl-cap: "Top 5 Zoning Districts by Net Development Capacity, Kent"
...
summarise(.groups = "drop",
  ...
  mutate(area_ndc_mi2 = poss_multiply(area, awm_ndc),
         area_ndc_mi2 = if_else(area_ndc_mi2 <=0,0,area_ndc_mi2),
         area_ndc_ft2 = area_ndc_mi2 * 27878400) |> 
   select(-area_ndc_mi2) |> 
  arrange(desc(area_ndc_ft2)) |> 
   slice_head(n = 5) |> 
...
t_mercerisland <- case_studies_transit_stations |> 
  filter(name %in% "Mercer Island") 

mercerisland <- 
  case_study_cities_no_water |>
  filter(name %in% "Mercer Island") 

p_mercerisland <- analysis_parcels_ndc |> 
...
We find the area-weighted mean net development capacity ($AWM_{NDC}$) for Mercer Island's single station area to be `r cs_awmndc['Mercer Island']` FAR. This metric summarizes HB 2160's effect on the *Developable* parcels in the city's station area, which includes `r cs_n_pct['Mercer Island']` of the station area parcels (`r cs_n_pct['Mercer Island']` parcels) and `r cs_area_pct['Mercer Island']` of the station area land (`r cs_area['Mercer Island']` mi^2^).

In @fig-results-case-studies-mercerisland-histogram, we observe the distribution of changes in development capacity that HB 2160 would have on *Developable, Affected* parcels in Mercer Island's light rail station area.
...
mercerisland_hist_subtitle <- "Area-weighted mean net development capacity ($AWM_{{NDC}}$) is {cs_awmndc['Mercer Island']} FAR" |> 
  glue() |> 
  TeX()
...
Displaying the net development capacity of individual parcels reveals a clear pattern around Mercer Island's light rail station area, as shown in @fig-results-case-studies-mercerisland-maps. 

```{r}
#| label: fig-results-case-studies-mercerisland-maps
#| eval: true
#| message: false
#| fig-cap: "Net Development Capacity, Mercer Island Station Areas"
...
Aside from the Town Center zoning district, which already allows buildings with up to 4 FAR, the station area parcels have an area-weighted average FAR of 0.49—significantly lower than the maximum FAR that HB 2160 would introduce. The total net development capacity for station area parcels zoned as "Single-Family" would be a significant increase, as shown in @tbl-results-case-studies-kent-by-district.


```{r}
#| label: tbl-results-case-studies-mercerisland-by-district
#| message: false
#| tbl-cap: "Zoning Districts by Net Development Capacity, Mercer Island"
...
t_redmond <- case_studies_transit_stations |> 
  filter(name %in% "Redmond")

# Redmond has several non-contiguous parts of the city.
# These will be dropped in order to allow the leaflet map
# to focus on the majority of the city
...
redmond <- 
  ...
  st_cast("POLYGON") |> 
  mutate(area_polygon = st_area(geometry)) |> 
  slice_max(area_polygon,n = 1) |> # keep only the main part of Redmond
  select(-area_polygon)

p_redmond <- analysis_parcels_ndc |> 
...
HB 2160 would affect several station areas within Redmond's city boundary, including four light rail stations and approximately 30 bus rapid transit stations.[^10] See @tbl-appendix-case-studies-redmond-stations-table in the appendix for a complete list of the station areas in Redmond.

[^10]: While most of the bus rapid transit stations are spatially distinct, there are several cases where two BRT stations appear to be in the same location. One example is Redmond Transit Center, where both Bay 1 and Bay 4 are included in the study's transit stop dataset even though they have (approximately) the same station area. This situation affect six BRT stops / three distinct station areas in total.
...
We find the area-weighted mean net development capacity ($AWM_{NDC}$) for Redmond's station areas to be `r cs_awmndc['Redmond']` FAR. This metric summarizes HB 2160's effect on the *Developable* parcels in the city's station areas, which includes `r cs_n_pct['Redmond']` of the station area parcels (`r cs_n_pct['Redmond']` parcels) and `r cs_area_pct['Redmond']` of the station area land (`r cs_area['Redmond']` mi^2^).

In @fig-results-case-studies-redmond-histogram, we observe the distribution of changes in development capacity that HB 2160 would have on *Developable, Affected* parcels in Redmond's station areas.
...
redmond_hist_subtitle <- "Area-weighted mean net development capacity ($AWM_{{NDC}}$) is {cs_awmndc['Redmond']} FAR" |> 
  glue() |> 
  TeX()
...
Examining the data grouped by station area type, we observe that while both station area types contain a similar number of *Developable, Affected* parcels, the *Large* station areas include substantially more land area that would be affected by the bill. Additionally, the $AWM_{NDC}$ of Redmond's *Large* station areas is substantially higher than the *Small* station areas (+1.90 FAR and + 1.15 FAR, respectively).
...
Displaying the net development capacity of individual parcels reveals a complicated pattern around Redmond's station areas, as shown in @fig-results-case-studies-redmond-maps. 
...
While there are a few neighborhoods that would not be affected, such as Redmond Town Center and the manufacturing park on the north, the majority of parcels within the transit station areas would have their maximum development capacity increased by HB 2160. For example, the Overlake Mixed-Use zoning district would see substantial increase to its total development capacity (see @tbl-results-case-studies-redmond-by-district).

```{r}
#| label: tbl-results-case-studies-redmond-by-district
#| message: false
#| tbl-cap: "Top 5 Zoning Districts by Net Development Capacity, Redmond"
...
t_shoreline <- case_studies_transit_stations |> 
  filter(name %in% "Shoreline")

# Shoreline has several non-contiguous parts of the city.
# These will be dropped in order to allow the leaflet map
# to focus on the majority of the city
...
shoreline <- 
  ...
  st_cast("POLYGON") |> 
  mutate(area_polygon = st_area(geometry)) |> 
  slice_max(area_polygon,n = 1) |> # keep only the main part of Shoreline
  select(-area_polygon)

p_shoreline <- analysis_parcels_ndc |> 
...
HB 2160 would affect several station areas within Shoreline's city boundary, including two light rail stations and approximately 30 bus rapid transit stations.[^11] See @tbl-appendix-case-studies-shoreline-stations-table in the appendix for a complete list of the station areas in Shoreline.

[^11]: While most of the bus rapid transit station areas are spatially distinct, the Aurora Village Transit Center constitutes two stations (Bay 10 and Bay 3) in the study data set even though they have (approximately) the same station area.
...
shoreline_hist_subtitle <- "Area-weighted mean net development capacity ($AWM_{{NDC}}$) is {cs_awmndc['Shoreline']} FAR" |> 
  glue() |> 
  TeX()
...
Displaying the net development capacity of individual parcels reveals a complicated pattern around Shoreline's station areas, as shown in @fig-results-case-studies-shoreline-maps. 
...
The vast majority of parcels within this city's station areas is zoned as "Low-Density Residential". The total net development capacity for this zoning district would be a significant increase, as shown in @tbl-results-case-studies-shoreline-by-district.

```{r}
#| label: tbl-results-case-studies-shoreline-by-district
#| message: false
#| tbl-cap: "Top 3 Zoning Districts by Net Development Capacity, Shoreline"
