...
case_study_results <- analysis_parcels_ndc |> 
  ...
  summarise(.groups = "drop",
            ...
            awm_ndc = weighted_mean(x = analysis_ndc_uniform,
                                    w = area_mi2),
...
case_study_results |> 
  ...
  tab_source_note(
    source_note = "Note: The figures in this table are limited to those parcels within station areas that are developable and impacted by HB 2160. They do not include all parcels within the station areas of each city."
  ) |> 
  tab_footnote(
...
cs_n <- cs_results_analysis_group |> 
  filter(analysis_type_uni == "Developable, Affected") |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"n") |> comma())

cs_area <- cs_results_analysis_group |> 
  filter(analysis_type_uni == "Developable, Affected") |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"area"))

cs_n_pct <- cs_results_analysis_group |> 
  filter(analysis_type_uni == "Developable, Affected") |> 
  (\(x) split(x, x$city))() |> 
  map(\(x) pluck(x,"n_pct"))

cs_area_pct <- cs_results_analysis_group |> 
  filter(analysis_type_uni == "Developable, Affected") |> 
  (\(x) split(x, x$city))() |> 
...
# max_ndc <- ceiling(max(na.rm = TRUE, analysis_parcels_ndc$analysis_ndc_uniform) * 2) / 2
...
p_kent <- analysis_parcels_ndc |> 
  ...
  select(area_mi2,
         district_name,
         net_dev_capacity = analysis_ndc_uniform,
         analysis_type_uni)
...
We find the area-weighted mean net development capacity ($AWM_{NDC}$) for Kent's station areas to be `r cs_awmndc['Kent']` FAR. This metric summarizes HB 2160's effect on the *Developable, Affected* parcels in the city's station areas, which includes `r cs_n_pct['Kent']` of the station area parcels and `r cs_area_pct['Kent']` of the station area land.
