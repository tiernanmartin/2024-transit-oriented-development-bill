...
Plotting the distribution of area-weighted net development capacity for this group of parcels, we observe two modal "peaks" at +1.5 and +2.5 FAR. The area-weighted mean net development capacity ($AWM_{NDC}$) is +1.35 FAR.
...
```{r}
#| label: results-histogram-station-areas
#| output: true
#| message: false
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Figure 3: Transit Station Areas, Central Puget Sound"
...
In order to get a sense for how the effects of HB 2160 may differ from place to place, we examine four cities as case studies: Kent, Mercer Island, Redmond, and Shoreline. These cities provide a range different contexts with which we can examine the data.

```{r}
#| label: results-case-studies-summary
#| message: false
#| tbl-cap: "Table 2: Case Studies"
...
```{r}
#| label: results-case-studies-analysis-group
...
```{r}
#| label: results-case-studies-kent-stations
#| message: false
...
```{r}
#| label: results-case-studies-kent-parcels
#| message: false
#| #| fig-cap: "Kent Station Areas Parcels"

p_kent <- p |> 
  filter(zoning_juris %in% "Kent") |> 
  select(zoning_new_max_far_additional)

p_kent_bbox <- st_bbox(p_kent)

kent <- ps_cities |> 
  filter(name %in% "Kent")


map_kent_p <- mapview(kent,
        col.regions = "grey",
        alpha.regions = .10,
        legend = FALSE) + 
  mapview(t_kent,
  ...
  mapview(p_kent,
          ...
          legend = TRUE,
          layer.name = "Change in FAR",
          popup = "zoning_new_max_far_additional") 
