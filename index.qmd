...
mode_pal <- brewer.pal(4, "Set2") |> 
  rev() |> 
  set_names(levels(case_studies_transit_stations$mode))
```
...
We find the area-weighted mean net development capacity ($AWM_{NDC}$) for Kent's station areas to be `r cs_awmndc['Kent']` FAR. This metric summarizes HB 2160's effect on the *Developable* parcels in the city's station areas, which includes `r cs_n_pct['Kent']` of the station area parcels and `r cs_area_pct['Kent']` of the station area land.
...
t_mercerisland <- st_join(t, case_study_cities) |>  
...
mercerisland <- 
  case_study_cities_no_water |> # use the 'no water' version
  filter(name %in% "Mercer Island")
...
t_redmond <- st_join(t, case_study_cities) |>  
...
t_shoreline <- st_join(t, case_study_cities) |>  
