...
HB 2160 would affect several station areas within Redmond's city boundary, including four light rail stations and approximately 30 bus rapid transit stations.[^8] See @tbl-appendix-case-studies-redmond-stations-table in the appendix for a complete list of the station areas in Redmond.

[^8]: While most of the bus rapid transit stations are spatially distinct, there are several cases where two BRT stations appear to be in the same location. One example is Redmond Transit Center, where both Bay 1 and Bay 4 are included in the study's transit stop dataset even though they have (approximately) the same station area. This situation affect six BRT stops / three distinct station areas in total.
...
```{r}
#| label: fig-results-case-studies-redmond-histogram-filled
#| output: true
#| message: false 
#| fig-height: 4
#| fig-cap: "Redmond Case Study: oning District by Net Development Capacity"
p_affected_redmond |>
ggplot() +
  aes(x = net_dev_capacity,
      weight = parcel_area_mi2,
      fill = fct_lump_n(zoning_district_name,5)) +
geom_histogram(binwidth = .5, 
               position = "stack", 
               alpha = 1) +
  theme(legend.title = element_blank()) +
  scale_y_continuous(labels = scales::label_comma()) +
  scale_x_continuous( breaks = seq(0, p_addtl_max, by = 0.5)) +
  labs(title = "Zoning District by Net Development Capacity",
       subtitle = "Redmond, Washington",
       y = "Land Area (sq. miles)",
       x = "Net Development Capacity (FAR)")
```
